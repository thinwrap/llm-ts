...
// it — no bundler in the loop. Bundlers (vitest) tolerate extensionless
// specifiers that Node's ESM loader rejects, so this is the only gate that
// catches a broken dist/esm emit. Runs offline; requires `npm run build` first.
...
const EXPECT = ['Chat', 'ConnectorError', 'OpenAICompatConnector'];
